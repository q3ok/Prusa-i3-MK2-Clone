...
uint16_t max_display_update_time = 0;
  ...
  #if ENABLED(DOUBLECLICK_FOR_Z_MOVE)
    void lcd_quick_move_z();
  #endif
  ...
  void lcd_goto_screen(screenFunc_t screen, const uint32_t encoder = 0) {
    if (currentScreen != screen) {
      ...
      #if ENABLED(DOUBLECLICK_FOR_Z_MOVE) || (ENABLED(DOUBLECLICK_FOR_Z_BABYSTEPPING) && ENABLED(BABYSTEPPING))
        ...
        if (screen == lcd_main_menu) {
          ...
          // Quick access to Z move enabled only when NOT printing
...
void kill_screen(const char* lcd_msg) {
  ...
  inline void line_to_z(const float &z) {
    ...
    void _lcd_calibration_menu() {
      START_MENU();
      MENU_BACK(MSG_MAIN);
      MENU_ITEM(function, MSG_CALIBRATION_Z, lcd_calibration_z);
      MENU_ITEM(function, MSG_CALIBRATION_MESH, lcd_calibration_mesh);
      END_MENU();
    }
  ...
  inline void manual_move_to_current(AxisEnum axis
    ...
    manual_move_start_time = millis()
	  #ifndef DISABLE_MANUAL_MOVE_DELAY
	    + (move_menu_scale < 0.99 ? 0UL : 250UL) // delay for bigger moves
    #endif
    ;
  ...
  void _lcd_move_xyz(const char* name, AxisEnum axis) {
    ...
    if (encoderPosition && !processing_manual_move) {
      ...
      #if ENABLED(DISABLE_MOVE_OFFSET_SELECT)
        move_menu_scale = 1.0;
      #endif
