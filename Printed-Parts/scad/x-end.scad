...
//Nut trap
 // Cylinder
   translate(v=[0,-17,0]) poly_cylinder(h = 10, r=12.5, $fn=25);
 // Hexagon
 //translate(v=[0,-17,0]) rotate([0,0,30]) cylinder(h = 8, r=8, $fn = 6);
 translate(v=[-6,-10.6,12]) rotate([0,0,48.2]) cube(size = [10,5,1], center = true);
}
...
// TR Nut trap
   // Hole for the nut
   // Modification by dulouie for TR8
    translate(v=[0,-17, -1]) poly_cylinder(h = 11.01, r = 5.3, $fn = 25); 

// Screw holes for TR nut
    // Modification by dulouie for TR8
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, 8, -1]) cylinder(h = 12, r = 1.55, $fn=25);
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, -8, -1]) cylinder(h = 12, r = 1.55, $fn=25);

// Nut traps for TR nut screws
    // Modification by dulouie for TR8
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, 8, 8]) rotate([0, 0, 0])cylinder(h = 3, r = 3.3, $fn=6);

    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, -8, 8]) rotate([0, 0, 30])cylinder(h = 3, r = 3.2, $fn=6);
    translate([-5.5,-17.2,8]) rotate([0,0,30]) cube([5,5,3]);
    translate([-0,-17.2,8]) rotate([0,0,60]) cube([5,10,3]);
}
